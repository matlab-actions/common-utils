...
describe("Coverage Data Retrieval Tests", () => {
    ...
    it("should return null when coverage file does not exist", () => {
        ...
        const result = codeCoverageSummary.getCoverageData();

        expect(result).toBeNull();
        expect(fs.existsSync).toHaveBeenCalledWith(
            path.join("/tmp/runner", "matlabCoverageResultstest-run-123.json")
        );
    });

    it("should return null when RUNNER_TEMP is not set", () => {
    ...
    it("should return null when GITHUB_RUN_ID is not set", () => {
