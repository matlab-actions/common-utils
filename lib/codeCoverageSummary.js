...
export function getCoverageResults(runnerTemp, runId) {
    let coverageData = null;
    const coveragePath = path.join(runnerTemp, `matlabCoverageResults${runId}.json`);
    if (existsSync(coveragePath)) {
        try {
            const coverageArray = JSON.parse(readFileSync(coveragePath, "utf8"));
            if (coverageArray.length !== 0) {
                coverageData = coverageArray[coverageArray.length - 1];
            }
        }
        catch (e) {
            console.error(`An error occurred while reading the code coverage summary file ${coveragePath}:`, e);
        }
        finally {
            try {
                ...
                console.error(`An error occurred while trying to delete the code coverage summary file ${coveragePath}:`, e);
            }
        }
    }
    return coverageData;
...
export function getCoverageTable(coverage) {
    ...
    const percentages = visibleColumns.map(col => `<td>${formatPercentage(col.data.Percentage)}</td>`).join('');
    const percentageRow = `<tr align="center"><td><b>Percentage</b></td>${percentages}</tr>`;
    // Build covered/total row
    const coveredTotals = visibleColumns.map(col => `<td>${col.data.Executed}/${col.data.Total}</td>`).join('');
...
//# sourceMappingURL=codeCoverageSummary.js.map