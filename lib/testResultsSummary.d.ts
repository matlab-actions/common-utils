import { CoverageData } from "./codeCoverageSummary.js";
export declare enum MatlabTestStatus {
    PASSED = "PASSED",
    FAILED = "FAILED",
    INCOMPLETE = "INCOMPLETE",
    NOT_RUN = "NOT_RUN"
}
interface MatlabTestDiagnostics {
    Event: string;
    Report: string;
}
interface MatlabTestCase {
    Name: string;
    Duration: number;
    Status: MatlabTestStatus;
    Diagnostics: MatlabTestDiagnostics[];
}
export interface MatlabTestFile {
    Name: string;
    Path: string;
    TestCases: MatlabTestCase[];
    Duration: number;
    Status: MatlabTestCase["Status"];
}
export interface TestStatistics {
    Total: number;
    Passed: number;
    Failed: number;
    Incomplete: number;
    NotRun: number;
    Duration: number;
}
export interface TestSession {
    FileName: string;
    TestResults: MatlabTestFile[];
    Stats: TestStatistics;
}
export interface TestResultsData {
    TestSessions: TestSession[];
    OverallStats: TestStatistics;
}
export declare function processAndAddTestSummary(runnerTemp: string, runId: string, workspace: string): void;
export declare function getTestResults(runnerTemp: string, workspace: string): TestResultsData | null;
export declare function addSummary(testResultsData: TestResultsData | null, coverageResultsData: CoverageData | null): void;
export declare function getTestHeader(stats: TestStatistics): string;
export declare function getDetailedResults(testResults: MatlabTestFile[]): string;
export declare function getStatusEmoji(status: MatlabTestStatus): string;
export {};
